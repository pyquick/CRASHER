...
const CASES: Record<string, { n: number; file: string; line: number | null; func: string; sev: string[] }> = {
  'game.txt': { n: 3, file: '[C]', line: null, func: 'error', sev: ['trigger', 'framework', 'source'] },
  'simple.txt': { n: 2, file: '[C]', line: null, func: 'assert', sev: ['trigger', 'source'] },
};
...
for (const [name, exp] of Object.entries(CASES)) {
  test(`parses ${name}`, () => {
    ...
    assert.equal(frames[0].language, 'lua');
...
test('detects via runtime hint', () => {
  const text = readFileSync(join(samplesDir, 'game.txt'), 'utf8');
  assert.equal(detectLanguage(text, HINT), 'lua');
});

test('auto-detects stack traceback without hint', () => {
  const text = readFileSync(join(samplesDir, 'game.txt'), 'utf8');
  assert.equal(detectLanguage(text, ''), 'lua');
});
